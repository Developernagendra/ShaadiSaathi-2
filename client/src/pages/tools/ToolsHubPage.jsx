...
import { FiCheckSquare, FiStar, FiCalendar, FiHeart, FiClock, FiMap } from 'react-icons/fi';
import api from '../../utils/api';

const categories = ['All', 'Planning', 'Budget', 'Guests', 'Vendors', 'Events', 'Travel', 'Design'];
...
export default function ToolsHubPage() {
  ...
  const tools = [
    ...
    {
      id: 'invitation',
      title: t('tools_hub.tool_invite_title', 'Wedding Invitation'),
      description: t('tools_hub.tool_invite_desc', 'Create and manage your wedding invitations.'),
      category: 'Design',
      icon: <LuMail />,
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] font-sans pb-20 overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-center gap-12">
            <div className="flex-1 text-center md:text-left">
              ...
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl md:text-5xl lg:text-6xl font-display font-black text-gray-900 mb-6 tracking-tight leading-tight">
                {t('tools_hub.hero_desc', 'Everything You Need to Plan Your Dream Wedding')}
              </motion.h1>
              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-gray-600 text-lg md:text-xl font-medium mb-10 max-w-2xl leading-relaxed">
                {t('tools_hub.hero_subdesc', 'Budget se lekar guest list, wedding checklist aur venue planning tak — sab kuch ek hi jagah.')}
              </motion.p>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-4">
                ...
                <Link to="/user/dashboard" className="w-full sm:w-auto px-8 py-4 bg-white text-[#C2185B] rounded-full font-bold text-lg hover:bg-pink-50 transition-all shadow-md border border-pink-100 text-center">
            ...
            <div className="flex-1 w-full max-w-lg md:max-w-none relative">
              ...
              {/* Decorative Elements */}
              <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 4 }} className="absolute -top-6 -right-6 w-24 h-24 bg-white rounded-2xl shadow-xl flex items-center justify-center text-4xl border border-gray-100 z-20">
                💍
              </motion.div>
              <motion.div animate={{ y: [0, 10, 0] }} transition={{ repeat: Infinity, duration: 5 }} className="absolute -bottom-6 -left-6 w-24 h-24 bg-white rounded-2xl shadow-xl flex items-center justify-center text-4xl border border-gray-100 z-20">
                📅
              </motion.div>
            </div>
          </div>
      ...
      <div id="tools-grid" className="max-w-7xl mx-auto px-4 py-20">
        <h2 className="text-3xl font-display font-black text-gray-900 mb-10 text-center">Featured Tools</h2>
        ...
        {filteredTools.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <AnimatePresence>
              {filteredTools.filter(t => t.featured).map((tool, idx) => (
                <motion.div
                  layout
                  ...
                  key={tool.id}
                >
                  <Link to={tool.path} className="bg-white rounded-[1.5rem] p-6 shadow-sm hover:shadow-xl border border-gray-100 transition-all duration-300 group flex flex-col h-full hover:-translate-y-1 relative overflow-hidden">
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl mb-6 shadow-sm bg-gray-50 text-[#C2185B] group-hover:scale-110 group-hover:bg-[#C2185B] group-hover:text-white transition-all duration-300 relative z-10">
                      {tool.icon}
                    </div>
                    ...
                    <div className="mt-6 flex items-center gap-2 text-[#C2185B] font-bold text-sm">
                      {t('tools_hub.open_tool', 'Open Tool →')}
          ...
          <div className="flex-1 text-center md:text-left">
            {isLoggedIn ? (
              ...
              <>
                ...
                <Link to="/auth/login" className="inline-flex items-center justify-center w-full md:w-auto px-8 py-3.5 bg-[#C2185B] text-white rounded-xl font-bold text-lg hover:bg-[#a3154d] transition-all shadow-md">
        ...
        <div className="bg-gradient-to-r from-[#8E244D] to-[#C2185B] rounded-[3rem] p-10 md:p-16 text-center shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link to="/user/dashboard" className="w-full sm:w-auto px-8 py-4 bg-white text-[#C2185B] rounded-full font-bold text-lg hover:bg-gray-50 transition-all shadow-lg hover:-translate-y-1">
